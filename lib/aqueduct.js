...
class Aqueduct extends FilteredEventEmitter {
  ...
  start() {
    ...
    for(let f of this.faucets) {
      this._startFaucet(this.tapEventSource, f)
    }
    this.started = true
  }
  ...
  _startPipe(tapEvents, pipe) {
    ...
    let remoteRead = remote;
    if(pipe.remoteRead) {
      remoteRead = this.remoteConnection[pipe.remoteRead]
      if(!remoteRead)
        throw new Error(`Invalid remoteRead collection ${pipe.remoteRead}`)
    }
    ...
    if(pipe.interval !== -1)
      this.flows[pipe.local] = this._buildFlow(pipe, remoteRead, local.upsert.bind(local))
    this._handleTap(tapEvents, pipe, remote, local)
  }

  _startFaucet(tapEvents, f) {
    ...
    const logger = loggerFactory('faucet', f.local)
    faucet({
      flow,
      runFlow: this.runFlow.bind(this),
      onReread: (data, delay) => this.queue.add({
        action: 'reread', data, type: f.local
      }, { delay }),
      logger
    })
    // just for re-read events
    this._handleTap(tapEvents, f, null, null)
  }
  ...
  _handleTap(tapEvents, pipe, remote, local) {
    ...
    tap(tapEvents, pipe, local && local.update.bind(local), remote, ack, log, events)
  }
